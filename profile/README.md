...
**[aowlsem](https://aoughwl.github.io/docs/aowlsem) — 54 commits. Imported templates and `{.untyped.}` generics were both instantiated by substitution-copy; both are now re-semchecked at the splice, and that is where the missing instances were.**
...
**Gates.** `tests/diff.sh` 675/675 corpus, built and run at `3407188`. New corpus cases lock char-const folding across a module boundary, block-body scoping (declarations and overload resolution), and `feature_untyped_generics`, promoted out of pending.
