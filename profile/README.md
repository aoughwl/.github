...
**[aowlsem](https://aoughwl.github.io/docs/aowlsem) — 18 commits.** Module-scale parity **73809 → 54218 tokens (−26.5%)** across the 46 stdlib modules, from four sem fixes; plus the gate that can see such a change at all, which did not exist this morning.

- **A resolved dot re-entering `semDot` came back out degraded.** An untyped imported template's expansion is substituted with already-semchecked arguments and then semchecked again — so `(dot (dot (hderef c.0) tc.0 0 "x") stmts.0 0 "x")` arrived twice, and the second pass replaced each mangle with `sourceNameOf` of itself: `tc.0` → `tc`, four children → two. 1282 occurrences in `semcore.nim` alone, its most frequent token difference.
- **A `var` template param takes a LOCATION, not the value context's deref.** Arguments are semchecked as expressions, so a `var` local arrived `(hderef dest.0)` and was spliced into a body that passes it to another `var` param. Together with the above: `semcore.nim` **32104 → 21272**.
- **An imported instance's type arguments were never recorded**, so inferring `T` over std/tables' `hashes: seq.0.I….tabcr240b1` bound nothing and instances came out `(at [].8.sysvq0asl .)`. The information was in the artifact all along, in the slot a generic uses for its typevars. **−8559 tokens**; with a duplicate-instance fix the 6-line repro went 1515 → **3**.
- **`moddiff.sh` computed per-module token counts and compared them to nothing.** Three separate changes were green on the 719-case corpus and on noabort while regressing whole modules; `tests/moddiff.baseline` caught each automatically, one run apiece, and two were reverted with their numbers written down.
