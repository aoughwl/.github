...
**[aowlsem](https://aoughwl.github.io/docs/aowlsem) grew a second half today: it stopped being only a checker and started being an [optimizer](https://github.com/aoughwl/aowlsem/blob/master/OPTIMIZER.md).** The from-scratch semantic checker is now **~30.5k lines** of self-hosted Nimony across **917 commits** — **67 of them today** — with the byte-exact differential corpus at **631 modules**. Everything below sits behind a separate command, so the checker's output is untouched and the parity gate that guards it never moves.

Why a separate command matters: aowlsem's entire correctness argument is that it emits *exactly* the bytes the reference compiler emits, byte for byte, on 631 modules. An optimizer's whole job is to emit different bytes. Those two claims cannot share an exit path, so they don't — `aowlsem m` checks and must match, `aowlsem opt` rewrites and must only preserve *meaning*. **Twenty-one passes** run today: constant folding across arithmetic, bitwise, shift and comparison operators; the algebraic identities (`x+0`, `x*1`, and the ones that discard an operand, which are only legal once you can ask whether the discarded half was harmless); constant `if` and `case` selection; unreachable code after `return`/`raise`/`break`; dead locals, dead assignments and dead private procs; constant and copy propagation; and inlining in three shapes. It runs to a fixpoint, because folding a condition is what exposes a dead branch, deleting the branch is what makes a local unread, and deleting the local is what makes a proc uncalled.

**The interesting part is not the passes — it is what it takes to believe them.** A dozen hand-written test programs will call an optimizer green, and today proved it: a scale gate that builds real programs three ways (the reference compiler's output, ours unoptimized, ours optimized), runs all three on the [aowli](https://aoughwl.github.io/aowli) interpreter and demands byte-identical output and exit status, **caught nine genuine miscompiles the small suite had passed** — an expression-`if` deleted outright, an inliner that accepted any three-statement proc as a one-expression one, arithmetic made observable by an overflow-flag pragma, inlined generic-instance bodies, and statements pasted into expression position. Each was narrowed to the responsible pass by switching passes off one at a time, then pinned with a regression program. The gate started the day comparing **103** real programs and ended it comparing **344** of 609 candidates, with every remaining skip written down by category — because each program recovered is one more the optimizer is proven safe on.

And it has to actually pay, measured rather than asserted: three nested one-line procs in a hot loop go from **360,003 calls to 3** and 505ms to 95ms (**5.3x**); a single hot one-liner, 120,003 calls to 3, 212ms to 77ms (**2.8x**); a partly-constant loop body, 118ms to 64ms (**1.8x**). On whole library modules the numbers are honestly small — around 1–3% of nodes removed — and the doc says so plainly, because a library is nearly all exported surface and exported means live: the deletion passes are not allowed to touch most of it.

**On the checker side, anonymous sum types were finished.** The type-declaration half landed yesterday; today closed construction and `of`-pattern matching, including the halves that only misbehave across a module boundary — instantiating an imported generic sum type (`Opt[T]`, `Result[T, E]`) left pattern bindings holding a field's *address* rather than its value, and the family's shared tag type was being re-declared in every importing module. Chasing that exposed a wider gap worth more than the feature: a local initialised by a call returning a generic application (`let r = p()` where `p` returns `Pair[int, string]`) had no type at all, so nothing about it resolved afterwards. Generic `ref` sum types with self-referential fields (`Tree[T]` holding `left, right: Tree[T]`) took a six-part fix. `std/opt` and `std/result` now check byte-exact both as the module that defines them and as a module that imports them.
