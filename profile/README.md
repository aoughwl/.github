...
**[aowli](https://aoughwl.github.io/docs/aowli-release) — 18 commits. Every defect below was a silent wrong answer: plausible output, exit 0, empty stderr.**
...
* **`advance()` and `complete()` returned nothing, in both engines.** Not because std/system's top level never runs — because `var scheduler: Scheduler = trivialTick` is a module-level **variable holding a proc, called through its own name**, and neither engine could call an imported global as a callee. The tree-walker's Symbol-callee path consulted `lookupCell` (locals only), so the name fell through to the routine path, resolved to no body and hit `failUnported`; the VM's `compileCall` checked `smap`/`gmap` (locals + main-module globals), compiled a bodyless routine and returned **nil in silence** — which is why a coroutine driven this way read as "printed nothing" rather than "unsupported". Both now fall back to the lazy imported-global slot: `globalInitCell` → `callClosure`, and `opLoadGlobalInit` + `opCallIndirect`.
* **The VM compiled `complete` as an unconditional `opCallNative`**, but the native only drives reified `vkCont` coroutines. A hand-built `Continuation(fn:…, env:…)` had no body to fall back to and reported an unported primitive, where the tree-walker falls through to system's own body. Now `opCallNativeGated`, split on `arg0.kind != vkCont` — the same split `evalCall` makes.
* **`setScheduler` wrote engine state and never system's `scheduler` global**, in both engines, so a program that installed a scheduler was still driven by `trivialTick`. The steps ran either way, so stdout was **identical** and only the scheduler's own bookkeeping differed — `ticks 0` against native's `2`. The tree-walker now runs the real body too; the VM got `opSetScheduler`, writing both views, with the global located from `setScheduler`'s own `scheduler = handler` assignment rather than matched by name, so it is necessarily the slot the reader resolves through `importedGlobal`. The gate case counts scheduler **calls**, not steps, because the steps were never the observable.
* **`--break-func` fired once per statement of the body.** `bkFunc` matches on the routine name and the predicate runs per statement, so a 5-statement routine called twice gave 10 hits of the same frame, a real routine spent the whole capture cap inside one invocation, and `continue` in a matched routine behaved like `step`. Now once per invocation (`Frame.fnBpFired`, reset by `acquireFrame`, so recursion re-breaks); `funcEntryOnlyMatch` bows out when a non-func breakpoint also matches the position, so `--break:FILE:LINE` inside the same routine still fires every time.
* **`readFile("data.txt")` failed on a path correct for the program**, because aowli is launched from wherever the caller sits — a temp nimcache under the tooling. `--cwd:DIR` / `AOWLI_CWD` (`progBaseDir`, `resolvedReadPath`) is tried **only after** the cwd-relative open fails, so no path that already resolved changes.
...
**Gates, second pass.** `crosscheck.sh` and `run.sh` defaulted their engine paths to the **absolute** `/home/savant/aowli/bin/aowli-{interp,vm}` while resolving everything else relative to their own root. aowli is developed across ~10 worktrees, so either gate run from any of them measured **master's** binaries. Not hypothetical: `crosscheck.sh all` from the `complete-integration` worktree reported **6 DIVERGE**, including the two `cps/tclosure_iter_*` tests that branch exists to fix — a plausible regression report for a build that was never under test. Both now default to `$NIFI_ROOT/bin`, matching `semantics.sh`, which had it right. Real figures after: `run.sh all` **434/434 all categories**, `semantics.sh` **4/4**, `dbg.sh` **15/15**, and on `complete-integration` with the paths pinned, `crosscheck.sh all` **457 AGREE-PASS / 19 AGREE-FAIL / 0 DIVERGE**. The three divergences on master are pre-existing, confirmed by rebuilding a baseline VM and reproducing them.

**`c3b572c` does not compile on its own.** It adds the only *call* to `isCellAddr`/`addrToCellPtr` in `interp.nim`; the only *definition* (`proc addrToCellPtr`, `flatmem.nim`) lands in `ae8c255`, the next commit — one commit each, verified with `git log -S` over both strings. Cherry-picking it alone left a branch non-building (`interp.nim(4011,18) Error: undeclared identifier: cp`), and `ae8c255` does not apply on top. The pair must move together, and `git bisect` across that range hits a build failure unrelated to whatever is being hunted.

**Standing.** Both engines agree on every runnable test. `master` and `complete-integration` have **both** moved: 29 commits ahead and 15 ahead respectively, each carrying correctness fixes the other lacks — VM closure capture, the user-scheduler trampoline and the OS namespace boundary on one; the `return`-value fix, cell-backed pointer addressing, the debugger line and `semantics.sh` on the other. There is no fast-forward, and cherry-picks conflict in `interp.nim` (master refactored the native intercepts to a `CallInfo.special` enum; the other branch still compares `ci.base`). Convergence is a merge with a behavioural gate on both sides. Arena slices 1–5a in: real memory block, graphs materialized at native layout, coherence after native mutation, pointer-bearing params crossing by arena address, `seq[T]` fields as contiguous element blocks. **5b is the remaining blocker** for near-native aowlsem debugging — a `Cursor` is an interior pointer into a `TokenBuf`'s element block carried independently of it, and it needs the arena backing those values so an address *is* a number.

**[aowlcode](https://aoughwl.github.io/docs/aowlcode) — 44 commits. One question applied to every tool: what is its verdict actually resting on? Each of these reported success, or a clean comparison, for work that had not happened.**

* **`nimony` exits 0 on an argument it rejects.** `nimony c --bogus-flag f.nim` prints 63 usage lines and **exits 0** — compiles nothing, no `Error:`, no crash marker, no artifact. So `compile` returned `ok:true` with an empty `diagnostics` list for a build that never ran: one typo in `extra_args`, or one bad line in a project's `.aowlcode.flags`, silently made a compile a no-op that reported success. The usage banner is now a failure spelling alongside `undefined reference` / `collect2:` / `[Bug] `.
* **`explain_failure` answered `OK: compiles clean` for failing compiles.** `if ok or errors.len == 0` derived the verdict from the *absence* of a diagnostic its parser recognised, so a timed-out compile read as success — from the tool whose only job is explaining failures. The verdict now comes from the compile's own `ok`, and `compile` gained `unparsed_output` (the toolchain's last 12 lines when nothing parsed; `raw:true` only ever showed the invocation).
* **Equivalence verdicts over runs that never ran.** `nif_run` reported `identical:true` for two variants that both **crashed** — its guard only covered a *silent* reference, so anything printing before dying passed as clean. `trace_diff` printed `behavior preserved at trace granularity` when **neither** run produced a trace (equal because both empty), and reported exit codes while excluding them from the verdict, so 0-vs-1 read as preserved.
* **ddmin bounded nothing.** `bisect` is O(n²) runs; its per-run `timeout` left 20 toggles × 120s = 40+ min, past the client's silence budget — the call is abandoned and every run wasted. Total `budget` added (600s, cap 900) returning the reached subset as `budget_exhausted`; same for `shrink`, which also never admitted hitting its 200-compile cap and returned a partially reduced file as `minimal_source`. Separately, **a killed run satisfies `exit_nonzero`**, so timed-out subsets had been recorded as reproducing without being observed to.
* **A stale server binary answered while every signal said healthy.** `build.sh` overwrites `bin/server` in place, so the process keeps a deleted inode at a path whose contents are now someone else's, under a matching version — path and version comparisons are blind by construction. Now `doctor.server_stale` + a session-banner warning, and `launch.sh` stopped serving stale *by design*: `AOWLCODE_BUILD_WAIT` (default 25s) lets a rebuild finish before the session commits to a binary.
* **The aowli binaries already hot-swap** — replacing `~/.aowl/bin/aowli-interp` or `aowli-dbg` takes effect on the next call, no reload or restart, because the path resolves per call and aowli output is never cached. Verified by swapping under a live server. Added: `doctor versions:true` identifies the live build (`aowli-dbg` by `--version`, `aowli-interp` by size+mtime, having none), and a paused `debug_session` — an already-running process of the old build — reports `binary_swapped`. Also fixed: `nif_run` copied only `*.s.nif`, leaving `.idx.nif` VFS indexes behind, so a module with sibling nimcache deps died on `vfs: open failed` — its stated purpose (verified against `~/aowlsem/nimcache`, 43 index files).
* **False failures cost the same as false successes.** nimony builds through a shared `nimcache_static` regardless of project, so two concurrent compiles overwrite each other's objects and the link dies with `undefined reference`. Two spurious failures in one day, each costing a re-run to disbelieve. Every nimony invocation — both servers, `build.sh`, test scripts — now serialises through one `flock`: serialise, not isolate, because a unique `--nimcache:` per call discards cache reuse and does not cover `nimcache_static` anyway.

**Gates.** The differential harness compared tool **names**, so both servers could advertise the same 26 tools with different contracts — 6 of 26 did. Fixing that exposed wrong `required` sets on 3 (`nif_file` demanded while `file` is a documented alias), and fixing that exposed a missing `action` enum on `debug_session`, leaving its 18 actions undiscoverable from the server that serves them. `parity.py` also printed `0 cases, 0 divergent` and exited 0 for a filter matching nothing, and reported its curated count as coverage while a separate file-sweep gate went unrun. `precompact-nudge` had **never worked** — `hookSpecificOutput` is rejected on `PreCompact`, so every compaction printed a validation error and discarded the reminder. `launch.sh` had no test at all, though one stray line on stdout corrupts the JSON-RPC stream for a whole session. Now: **197 curated + 116 sweep differential cases, 107 unit checks, 39 end-to-end checks**, plus all 8 hooks smoke-tested from `hooks.json`.

**Messaging between instances.** Several Claude Code sessions run at once with no channel between them. `/listen <label>` claims `~/.claude/msgbus/inbox/<label>/` and arms the **Monitor** tool on `aowl-listen`, so an arriving message becomes an event in that session's conversation — an instance cannot poll, it acts only on a turn, so delivery has to push. Transport is a maildir: the sender writes a temp file and `mv`s it in (atomic, no lock), the listener drains oldest-first into `read/`. A message to a label nobody has claimed queues instead of failing, and is delivered when that instance runs `/listen`; `aowl-msg --list` reports labels, liveness, and queue depth. The reply leg is the same command with `--from`, so a worker reports its own fix back with no human turn in between. A message lands between tool calls, never mid-call, and never authorises a force-push, release, or delete — the worker asks its own human. Also `/update-blog`, which writes this file: which of the two `.github` clones is live, the append-to-today rule, and that every count comes from `git log` or a gate that ran.

* **One inbox per repo name** — `aowli`, `aowlsem`, `aowlcode`, `playground`. `/work <repo>` claims that label, drains the queued backlog *before* any self-directed work, then works the repo without stopping to ask what is next. Requests close in two steps, `--ack` on pickup and `--done` with the commit SHA and the check's PASS/FAIL, so a sender can tell *seen* from *fixed*.
* **A live PID is not proof the inbox is being drained.** `--alive` reported a SIGSTOPped listener as healthy while its queue grew. The listener now rewrites a heartbeat every poll: fresh → `live`, stale → `wedged`, PID gone → `dead`, no heartbeat file at all → `live?` (a listener started before heartbeats existed — `--ping` refuses that one, since it has no ping branch and would surface the ping as a message in that session). `--ping` is the proof: a round-trip the listener process answers and deletes itself, so a liveness check **costs the other instance no tokens and never enters its conversation**.
* **The loop breaker was a sentence in a prompt.** Nothing stopped two instances acknowledging each other indefinitely. Every message now carries `chain:` and `depth:`; a send to a label that recently messaged this one continues that chain one deeper, and past `AOWL_MSG_MAXDEPTH` (6) it is refused. Depth counts **unanswered repeats, not just inbound replies** — the first version tripped on one direction only: with its peer refused, a sender's inbound record never advanced, so it re-sent at a fixed depth forever. A parent record older than an hour starts a fresh chain; ceiling of 30 messages per label per 60s.

**[serve](https://aoughwl.github.io/docs/net-stack/serve) — 25 commits. h2spec 95/146 → 146/146: the 95 was a concurrency defect, not a protocol one. The reactor then gained TLS, timers, a stop, an outbound client and produced bodies.**

* **The recorded reason for 95/146 was wrong.** It was logged as "http2.nim never answers a violation with GOAWAY". Every failing section passes when run alone — `h2spec http2/8.1.2` is 16/16. `serveHttp2`'s accept loop runs a whole session to completion before accepting again, so one connection left open wedges the listener and every later case times out (51 × `Error: Timeout`, 153s of them). Diagnostic: run a conformance suite section-by-section against all-at-once; a failure that appears only in aggregate is the concurrency model, not the protocol code.
* **`nghttp2_session_mem_recv` may consume less than it is given.** It was fed once per read and its return ignored, so a frame in the tail of a read was dropped without trace — the cause of 5.1.1 hanging, since the offending HEADERS arrived in the same segment as the valid one.
* **`close()` with unread bytes queued makes the kernel send RST**, which h2spec saw instead of the GOAWAY it was owed (3.8, 7.1, and over TLS 6.3, 6.4). Teardown is now `close_notify` → `shutdownTcpWrite` → bounded drain.
* **nghttp2 discards a HEADERS naming an already-used stream id in silence** and never reaches `on_begin_headers`, so RFC 7540 §5.1.1 cannot be enforced there. `onBeginFrame` tracks `lastRecvStreamId` and calls `nghttp2_session_terminate_session(NGHTTP2_PROTOCOL_ERROR)`.
* **`serve/reactorh2.nim`** drives h2c on the reactor over a transport-agnostic seam in `http2.nim` — `h2OpenSession`/`h2Feed`/`h2NextOut`/`h2Idle`. Sessions live in a fixed global table (`MaxH2Conns` 128) because nghttp2 keeps the `user_data` pointer and a coroutine local is not address-stable; overflow is counted by `h2RejectedConns()`. Suite runtime 153s → 0.72s.
* **The reactor had no clock**: `epollWait(…, -1)` blocks forever, so a coroutine parked on a socket that never became ready stayed parked for the process's life. `setIdleTimeout` arms a `CLOCK_MONOTONIC` deadline at `park` and disarms it on readiness — idleness, not connection age. Expiry **shuts the socket down** rather than resuming with an error, so the coroutine takes the EOF path it already has and no server needed new control flow. 60s HTTP/1.1 and HTTP/2, off for WebSocket.
* **`awaitReadableFor(fd, ms, timedOut)`** is the other timer: it resumes instead of killing, which is what a protocol servicing its own loss/idle/PTO timers needs. `serveH3Reactor` gave up its private epoll loop for it, and `spawnH3` attaches a QUIC server to a reactor someone else owns.
* **`serve/reactorall.nim`** — `serveAllReactor(port, cert, key, handler)`: TLS/TCP with ALPN dispatch and QUIC/UDP on the same port number, one handler, one OS thread, `Alt-Svc: h3=":<port>"` on every TCP response. HTTP/3's `(meth, path, body)` is adapted to `Request`/`Response` there rather than pushed onto the caller.
* **Every reactor server was plaintext-only.** `serve/asynctls.nim` maps `tlsWantRead`/`tlsWantWrite` onto `EPOLLIN`/`EPOLLOUT`; the park direction is TLS's choice, not the operation's. The accepted fd must go non-blocking **before** `wrapServer`, which otherwise runs the whole handshake inline on the reactor thread. `serve/asyncconn.nim`'s `Conn {fd, isTls, tls}` then keeps one connection body per protocol instead of two — a runtime branch, since the await primitives must be templates. New: `serveHttpsReactor`, `serveWssReactor`, `serveHttp2TlsReactor`, `serveHttpsAlpnReactor`.
* **Each TLS entry point built its own `TlsContext`**, putting versions, cipher suites, groups, SNI certificates and resumption out of reach — the same defect the roadmap records against blocking `serveTls`, reintroduced in the async servers. All of them now take a context overload; `serveTls`/`serveTlsConcurrent` too. Verified reaching the wire: a TLS-1.3-only context refuses `--tls-max 1.2` (curl 35) while an h2 client still gets HTTP/2.
* **`run()` could not be stopped**, so ending a server meant cutting a response in half. `requestStop(graceful)` writes 8 bytes to an eventfd the loop also watches — the only work a signal handler may do, and the only way to interrupt a blocked `epoll_wait`. Graceful closes the listeners (`registerListener`) and returns when the last in-flight connection closes.
* **A failed `accept()` was a bare `continue` in `poolWorker`** and a no-op branch in `serve`/`serveTls`: a dead listener span every worker at 100% CPU indefinitely, and `runPool` joined threads with no exit. Both loops now end on a hard accept error; `stopServing()` shuts down the registered listeners under them. `ReadTimeoutMillis` and the request/keep-alive caps became defaults (`setServeReadTimeout`, `setServeLimits`, `setReactorLimits`).
* **The stack served asynchronously and could only call synchronously.** `serve/asyncclient.nim` adds `awaitConnect`/`awaitFetch` — connect, TLS handshake, request, response, each step suspending. `timeoutMs` is armed **before** the connect, since a host that drops the SYN never makes the socket writable. DNS still blocks: `getaddrinfo` has no non-blocking form worth the name.
* **HTTP/2 sent exactly one SETTINGS entry.** `H2Settings`/`setH2Settings` cover MAX_CONCURRENT_STREAMS, INITIAL_WINDOW_SIZE, MAX_FRAME_SIZE, HEADER_TABLE_SIZE and MAX_HEADER_LIST_SIZE; a 0 field keeps nghttp2's default.
* **`submitResponseFor` trimmed response headers past 39, silently.** A dropped `Set-Cookie` is a wrong response that looks fine. 128 slots now, overflow answers 500, `h2HeaderOverflows()` counts it.
* **Static serving ignored the request**, re-sending every unchanged asset in full. `staticResponseFor` adds ETag (`size-mtime`, one stat), `Last-Modified`, `If-None-Match` → 304, ranges → 206/416, and a counted size cap. `httpDate` is civil-from-days integer arithmetic, checked against RFC 7231's own example date.
* **`getLastModificationTime` returns nanoseconds.** Fed to the formatter it emitted `Last-Modified: Mon, 02 Jun 56583934092 19:58:27 GMT`. `epochSeconds` classifies the magnitude (ns/µs/ms/s). It got through because the test asserted a proxy — that the header existed — not the property, that it names the file's mtime; both tests now check the date.
* **Every response path materialised the whole body.** `serve/stream.nim` is a pull producer, `proc(st: var StreamState; chunk: var string): bool {.nimcall.}`, called by the connection coroutine until false — pull because a handler cannot suspend and so cannot write. `contentLength >= 0` sends `Content-Length` and raw pieces; -1 is chunked on HTTP/1.1 and close-delimited below it. `fileStream`, `sseEvent`/`sseStream`, and `staticStreamFor` (validators and ranges, no cap needed).
* **WebTransport streams** (bidi + uni) close out WT: the initiator prefixes varint `0x41`/`0x54` plus the session id, the shim routes them itself, and a stream that turns out to be plain HTTP/3 has its held opening bytes replayed into nghttp3. The QUIC shim also counts every bounded-resource overflow (`AqStats`, 7 counters) instead of dropping in silence.

**Gates.** 21 scripts in `tests/`, 15 added today. **h2spec 146/146 h2c and 146/146 over TLS** (baseline in `tests/h2spec.sh` raised 95 → 146; it builds both servers and mints its own cert). `reactor_stream_e2e.sh` measures the streaming claim rather than restating it: 128 MiB byte-exact with the server's **peak RSS 6056 kB**. `reactor_proxy_e2e.sh` measures suspension the same way: 12 concurrent requests through a 0.5s upstream in **0.53s** (blocking would be ~6s), and a black-hole upstream answered 502 at its 1s bound. `tserve_stop.nim` was verified by putting the bug back — exit 124 with the old worker loop, pass with the fix. Unchanged: echo 300/300, HTTP/1.1 300/300, HTTPS 150/150 across 30 TLS conns, WS 160/160 + 19/19 conformance, wss 80/80, H3 20/20 + aioquic third-party interop, WT + WT streams, ALPN dispatch, all-protocols.

**Standing.** One thread now serves HTTP/1.1, HTTP/2 and HTTP/3 from one handler on one port, with TLS, idle timeouts, graceful shutdown, an outbound client and produced bodies. Remaining: a producer runs on the reactor thread and cannot pace itself (no timer wired into the stream path), handler-execution timeouts, and the `quicglue.c` bounds. `tests/lib.sh` retries a build once and says so — `ld: cannot find ~/nimony/nimcache_static/static.o` is a shared-directory race in nimony, mitigated here, not fixed.

**[http](https://aoughwl.github.io/docs/net-stack/http) — 2 commits.**

* **The package could build a `Response` and parse a `Request` — everything a server needs, nothing a client does.** `parseResponse` is the mirror: status line, headers, body, with `headerValue`/`hasHeader` overloads. Transfer framing is deliberately not applied (a chunked body comes back chunked; `decodeChunked` is the caller's next step), which is what lets a streaming client act on headers before the body arrives. `parseStatusLine` and `responseHeaderEnd` ship alongside for the incremental case. `serve/asyncclient.nim` is built on the three.
* **Header count and body size were unbounded**: `feed` grew `bodyBuf` forever and a declared `Content-Length` was believed unquestioned. 128 headers, 64 MiB body, `withBodyLimits`/`limits()`, 431/413.

**[aowlsem](https://aoughwl.github.io/docs/aowlsem) — 56 commits. Imported templates and `{.untyped.}` generics were both instantiated by substitution-copy; both are now re-semchecked at the splice, and that is where the missing instances were. Two constructs that had been copied through as raw trees — `typeof(expr)` and a macro's body — now resolve.**

* **An `{.untyped.}` imported generic stores a body that is name-bound but unchecked, and copying it left the dirty templates inside it unexpanded.** bitabs' private `getOrInclImpl()` holds the whole open-addressing loop, so every generic reachable only through it was never instantiated: **nifcore minted 76 instances where the oracle minted 139**. `emitInstanceUntyped` copies the signature (fresh param symbols, typevars replaced), defines the params by base name so the body's own references type, then semchecks the body. A dirty template is called BY SYMBOL, so a private one is registered under its full mangle (`privateImportedTemplates`) — by source name it would hide a public one. Opt-in per routine, or module-wide with `{.feature: "untyped".}`, which also makes every `template` in that module untyped.
* **`importedTemplates` mapped a name to ONE decl.** Enough to expand, but an `(ochoice …)` in an untyped body lists them all, and system spells two of its five `len`s as templates (`len.0.`/`len.1.`), so one was always missing from std/strbasics' choices. A parallel table keeps every mangle: **strbasics 95 → 92, bitabs 603 → 599, nifcore 18762 → 18739, strutils 3694 → 3691** diff tokens.
* **A template's own locals were not renamed hygienically**, and the rename mints from the BASE name. An untyped body binds names across scopes, its ochoice order is composite (locals first, then imports), a name declared inside it shadows the scope and stays bare, and a dirty EXPRESSION template — like a `runnableExamples`-style block argument — is semchecked at the splice rather than copied.
* **A `concept` was parsed, emitted and used for overload filtering, but never ENFORCED.** `proc sum[T: Addable](a, b: T): T = a + b` instantiated happily with an `Obj` that has no `+`. Every diagnostic is switched off inside an instance (`inInstanceEmit`) deliberately — an instance is copied, re-mangled source, and reporting from one produced false positives on valid code — so the requirement list is checked directly instead: `semProc` records each requirement signature under `inConceptReq`, `semTypeDecl` files them under the concept's own symbol, and `requestInstance` asks per constrained typevar whether every requirement resolves with `Self` substituted. E0282, reported at the call.
* **A nested generic's instance was drained at module level, where the enclosing routine's locals do not exist.** `tgeneric_closure`'s `proc inner[T] {.closure.} = discard x` emitted the capture as a bare unresolved `x` while the call named the instance. `requestInstance` holds those requests in `c.nestedInstReqs`; `semProcBody` splices them in place, immediately before the nested decl, while the enclosing scope is still live.
* **`build.sh` installed a binary this run had not linked.** It copies the newest `aowlsem` in the nimcache, which is not the same thing. `~/nimony/nimcache_static/static.o` is SHARED by every nimony link on the machine — the path comes from the nimony install, so `--nimcache:` cannot isolate it — and nimony's own test runner deletes it, so a concurrent link dies with `cannot find static.o`. That is loud only because nifmake exits nonzero: **nimony exits 0 on a bad flag**, and then the previous run's executable is copied and every gate afterwards tests the OLD binary while the script prints "built". Now asserts the artifact is newer than every source that went into it.
* **There was no way to ask "is aowlsem green?".** `tests/all.sh` runs unit, check, diff, beat, nofp, diag_gate, explain_gate and e2e, cheapest first, one exit code — and its header names the harnesses left OUT and why (moddiff, sysdiff and verdict are exploratory, with no fixed denominator, so they cannot pass or fail). `beat.sh` had sat red at 2/4 on two stale snapshots because nothing ran it. Verified by putting the bug back: a corrupted snapshot makes `all.sh` exit 1 and report `FAIL beat.sh — 3/4`. `diff.sh` now separates "no oracle" from "output differs", and COVERAGE.md's status block was four hundred cases out of date.

* **`typeof(expr)` was copied into the type slot as a tree.** `var a: typeof(f())` emitted the whole `(call typeof (call f))` where the oracle has `(i 64)`. It is a demand, not a type expression: `semType` hands the operand to `typeOfValue` and emits the answer. Three things had to hold. `typeOfValue` could not type an ordinary CALL — it knew builtin conversions and nothing else, so the query answered unknown and the slot came out `.`; a call's value has the callee's return type. It could not type an INFIX/PREFIX either — an arithmetic operator yields its operand type, a comparison yields bool regardless. And the callee slot is **not an Ident**: nifler hands back its disambiguated `typeof~6`, a post-sem body carries `typeof.0.sys…`, so a `strVal(cur) == "typeof"` test never fired at all. `calleeBaseName` truncates at whichever separator is present. Not applied inside a generic/template/macro decl, where surface syntax is preserved.
* **nimsem emits some macro bodies verbatim and semchecks others**, and neither rule alone passes: emitting every body verbatim fixes the parameterised macros and regresses the zero-arg ones, semchecking every body does the reverse. The predicate is the parameter list, not a heuristic — a macro parameter is `untyped`, so a body reading it is untypable and nimsem does not try (`result = newCall(ident"+", [n, n])` stays raw because `n` is untypable), while a zero-arg body is semchecked like any routine: mangled locals, `hcall toOpenArray`, trailing `ret`. `macroBodyStaysRaw`, computed next to `vParams` in `semProc`. The slot is never run from there — the macro plugin is built from the pre-sem nifler decl — so it only has to match the oracle.

**Gates.** `tests/diff.sh` 675/675 corpus at `3407188`, **677/677** at `8080d38` measured in an isolated worktree. New corpus cases lock char-const folding across a module boundary, block-body scoping (declarations and overload resolution), `feature_untyped_generics` promoted out of pending, and `typeof` over a var, an object field of each scalar kind, a call, an infix, a comparison, an object value and a proc RETURN type. New probes cover the parameterised macro side: one argument, two arguments, `strVal` on an ident argument, iterating an argument's children, matching an argument's kind, and returning a statement list. Rest of `tests/all.sh` on the day's working tree: check 400/400, beat 4/4, nofp 35/35, diag_gate 175/175, explain_gate 93/93, e2e 4/4.

**M4 magics, measured rather than assumed.** `defined()` is already byte-exact. `compiles()` is not implemented — nimsem's `semCompiles` sems into a scratch buffer and scans for an `(err …)`; for aowlsem a trial sem consumes `c.counters` mangle numbers, so it needs a snapshot/restore or every later mangle desyncs. `declared()` is not implemented and does **not** follow Nim 2: the oracle answers true for a proc declared in the module and for `int`, false for a module-level `var` and for imported `echo`. A `converter` has no oracle at all — nimony rejects `converter toInt(c: Cel): int` with "cannot attach converter to type int64".
