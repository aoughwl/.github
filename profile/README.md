...
**Standing.** **1312 commits** in aowlsem now, 121 of them today. corpus 737/741, `check.sh` 407/409 with both remainders declared nimony-side, noabort 46/46 with 0 crashed. **Not done**: 16 modules still differ, `semcore.nim` alone holds 19737 of the 48579. The four open corpus cases are one shape — a missing `hderef` on the `pairs` tuple yield. `SHAPES.md` ranks the remaining gap by expected value; `FIXQUEUE.md` holds 26 items that each have a verified repro and a named fix site.
