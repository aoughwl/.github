...
**The productive gate today was the broad-module differential, not hand-written probes.** `tests/moddiff.sh` compiles a real project into a fresh nimcache, re-semchecks every module for which the oracle produced both a `.p.nif` and a `.s.nif`, and canon-diffs each one. Hand-written feature probes had gone several rounds finding nothing — that seam is saturated. Forty real modules found a day's work in one run. It now stands at **18/40 byte-exact, 0 crashes**, with the rest quantified in tokens rather than described.

**The dominant class was not missing features. It was *when* an instance gets minted** — three separate rules, each worth thousands of tokens:

* A generic **type** decl's body keeps `(at G …)` raw. A generic **routine's signature** does not: it is shared by every instantiation, so it cannot wait for one, and `proc parseNum[T: SomeInteger](s: openArray[char])` (std/parseutils) instantiates `openArray[char]` at decl time.
* Inside a generic decl, a call over still-abstract operands stays **symbolic**. `t[k] = v` in `proc addTo[K: Keyable, V](t: var Table[K, V]; …)` is `(call []=.0.tabcr… t k v)` — the generic symbol, no instance, and **no `haddr`**. Instantiating it there dragged in the operator's entire lifetime-hook cascade: **5922 tokens on a two-line program**, and the same shape throughout `std/sets` (**7530 → 147**).
* `typeVarLike` — consulted from about twenty-five places, all of them asking "is this a type variable?" — **guessed from spelling**: one uppercase letter. `BiTable*[Id, T]` has two type parameters and it recognised one. Every typevar mint is now recorded, with spelling kept only as the fallback for an imported generic whose typevars this module never minted. std/bitabs **5863 → 1236**. The same test also has to recurse: `seq[(K, V)]` is not concrete just because a tuple sits on top.

**Commit #1000 fixed the ordering assumption underneath all of that.** Type instances are emitted in *request* order, and a dependency is always requested *by* the body of the thing that needs it — so it is emitted later. `HashSet[T] = object; t: Table[T, bool]` therefore had its field scanned before `Table[TagId, bool]` had registered any hooks: the field read as unmanaged, the HashSet instance got no lifetime hooks, and neither did any object holding one (std/optcore's `OptTags`). Reordering emission is **not** the answer — the oracle emits HashSet's instance before Table's too. nimsem decides off the *declaration*, so we now do: a generic whose body structurally holds a lifetime (a `string`, a `seq`, a `ref`, or a field of another such type) pre-registers its instance's hook names at request time. The predicate only ever answers "yes" on evidence, and it is self-consistent by induction. std/optcore **18369 → 16052**.

**A module-qualified name was unhandled in three separate emitters.** `import nifreader as rd` then `rd.ExpandedToken` hit `typeOfExpr` (which had no `dot` case at all, so the decl's type slot came out `.` and its zero a scalar), `semType` (parameter position), and the `case` of-value list (emitted structurally, so `of rd.EofToken:` kept the raw `(dot rd EofToken)`). One shared test resolves all three: the left side of the dot is nothing in scope, which is exactly what an import name is. Relatedly, `from std/dirs import walkDir` now really is selective — loading the whole module registered `dirs.getCurrentDir(): Path` alongside `ospaths2.getCurrentDir(): string`, and we were picking the wrong one.

**Smaller parity fixes, each pinned to a real program rather than invented:** a `distinct` over a primitive compares with the base's magic keyed on its *own* symbol — unless the type declares its own, and `!=`/`>`/`>=` cancel on the operator they *derive from* (lineinfos' `x != NoFile` calls the borrowed `==`); an **anonymous** routine must never consume a prescan slot even at depth 0, or a lambda in a global initializer takes the next proc's name and parameters (nimony's own `src/lib/vfs`); a proc-type's parameters are numbered at the **type decl's** source position, for an object field and not just a `type F = proc(...)` alias; a **user-declared** lifetime hook suppresses the whole fieldwise synthesis, and `=sink`/`=trace` count as one; `{.push header: … .}` applies to every decl **and every parameter** until the pop, with a string const for a value; ordering two pointers (`<`/`<=`, not `==`) is an address compare at `(pointer (unchecked))` with both operands converted; a prefix operator can be a **template** (`^p` in nifreader), which expansion only handled in call position; a **named argument may skip a defaulted parameter** — the reorder demanded exact arity, bailed, placed the named value positionally and appended the wrong default at the end; and four folds that were simply absent — set consts by name, float consts, `(par …)` in a `when` condition (`not (defined(cpu16) or defined(cpu8))` was dropping all of std/widestrs, **5009 → 415**), and an `{.untyped.}` template body, which nimsem still open-binds.

**One thing is written down as not reproducible, rather than chased.** nimsem lists an `(ochoice …)` in a hash order — system's `+` comes out `3, 6, 12, 10, 14, 0, 15, 2, 9, 5, 13, 4, 7, 16, 11, 8, 1`, which is not declaration order, not the `.s.idx.nif` order, and not sorted. We emit the right set, sorted, and the commit says so.
