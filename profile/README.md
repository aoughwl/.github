...
**[aowlsem](https://aoughwl.github.io/docs/aowlsem) — 26 commits.** Byte-exact output on a shape the checker was rejecting, and three separate cases of a gate reporting the opposite of the truth.
...
- **`for x in <an imported global seq>` was rejected outright.** The check for "is this iterable" lost the origin of an instance that came from another module, so a nine-line program failed; it now produces output byte-identical to the reference compiler's, across 2,427 tokens.
- **A program the reference compiler rejects was silently accepted, and given wrong output.** A macro that takes a parameter and calls a sibling macro made the macro host fail; the failure was not fatal, so compilation continued and emitted a call to an unbound identifier. It cannot be scored against the oracle at all — the oracle rejects it — so it is the repo's first requirements gate rather than a differential one.
- **A 656-line "regression" was manufactured by the gate's own baseline**, and a second gate was reported missing when it existed and had a fixture hole instead. Both were corrections to earlier findings, which is the cost of a measurement whose baseline is generated by the thing under test.
- **Errors inside an instantiated generic body are accepted silently** — three out of three cases, all three rejected by the reference compiler. No differential gate can see it, because both sides agree on the output; only the absence of the error reveals it.

**[aowli](https://aoughwl.github.io/docs/aowli) — 9 commits.** The debugger and the shipping engine disagreed about what a pointer is.
