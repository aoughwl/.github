...
- **Twice today a run of attempts came back changing nothing, and both times the nothing was the finding.** Six fixes in six places moved the output by exactly zero bytes; separately, six recorded diagnoses of the defect below were all wrong, each inferred from something that *failed to happen* under a debugger — one of them the debugger reporting a false negative (see aowlcode). Both were settled by stopping on the line that gave up and reading the values it held. A change that moves the number by nothing is evidence about which code runs, not about the condition you wrote.
...
- **A test that reproduces its input byte for byte cannot tell you the parse tree is right.** Deleting the rule that stops HTML from reading `<script>` contents as markup left that test passing **44 of 44** — every byte still present, merely filed under the wrong node. Tests that assert structure instead (how many nodes of each kind, how deeply nested) found a real defect on their first run: a closing `</script>` left the tokenizer in the wrong state and it consumed the rest of the document as opaque text, byte-exactly the whole time.
...
- **The cost is that a false negative is indistinguishable from evidence.** On a 432-hit run, an exact-equality check against a value a print statement proved was passing through that very function failed identically — so the miss read as a fact about the compiler and sent a day's search into the wrong half of it. Quotes are now stripped, `==` accepted, and an operator that cannot be honoured is rejected outright: a rejected filter is recoverable, a silent one is not. Of six new assertions, five fail against the previous parser.

**Across the org: 60 repositories, 4,450 commits, 591,085 lines of Nim** in 5,307 files. Scope, since a number without one is decoration: repositories on GitHub under the org, counting tracked `.nim` files only — tests and fixtures included, documentation and other languages not — and excluding our forks of the Nim-language compiler this all builds on, which are another 1,764 commits of code we did not write.
